...
void setUI(Record record, Ui::MainWindow* ui);
...
void setUI(Record record, Ui::MainWindow* ui) {
    ui->nameLabel->setText(QString::fromStdString(record.name));
    ui->phoneLabel->setText(QString::fromStdString(record.phone));
    ui->sidLabel->setText(QString::number(record.SID));

    int newRowCount = record.grades.size();
    ui->tableWidget->setRowCount(newRowCount);

    for (int i = 0; i < newRowCount; i++)
    {
        QString courseName = QString::fromStdString(record.enrollments[i]);
        QTableWidgetItem *course = new QTableWidgetItem(courseName);
        ui->tableWidget->setItem(i, 0, course);

        QString gradeNum = QString::number(record.grades[i]);
...
void MainWindow::on_action_Open_Database_triggered()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        db = stringToRecords(fileContents.toStdString());
        currentRecord = 0;
        maxRecords = db.size();

        ui->buttonPrev->setDisabled(!(maxRecords > 1));
        ui->buttonNext->setDisabled(!(maxRecords > 1));

        setUI(db[currentRecord], ui);
...
{
    currentRecord--;
    if (currentRecord == -1) currentRecord = maxRecords - 1;
    setUI(db[currentRecord], ui);
}
...
{
    currentRecord++;
    if (currentRecord == maxRecords) currentRecord = 0;
    setUI(db[currentRecord], ui);
}
